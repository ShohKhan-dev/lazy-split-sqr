...
def list_user_depts(group_id: int, user_id: int, db: Session = Depends(get_db)):
    ...
    if not depts:
        raise HTTPException(status_code=404, detail="No depts found for the given user ID")
    return depts
...
def update_dept_amount(dept_id: int, dept_paid: DeptPaid, db: Session = Depends(get_db)):
    ...
    if existing_dept.amount <= 0:
        ...
        return {"message": "Dept updated successfully, amount left: {}".format(existing_dept.amount)}
